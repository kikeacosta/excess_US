...
db_deaths <- tibble()
for(y in 2015:2019){
  temp <- read_tsv(paste0("Data/Underlying Cause of Death, ", y, "10x.txt"),
...
unique(db_deaths$`Five-Year Age Groups`)
...
test <- 
  db2 %>% 
  complete(State, Age, Sex, Year, Month, fill = list(Deaths = NA))
...
db_w4 <- read_rds("Output/weekly_deaths_by_sex_2015_2019.rds")
